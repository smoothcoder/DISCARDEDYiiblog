...
public function actionCreate()
	...
	$model=new File;
		...
		if(isset($_POST['File']))
		{
			...
			if($model->save())
			{
				...
				$this->redirect(array('files/admin'));
			}
	...
	//             {
	// 		$image=Yii::app()->image->load($model->name->getTempName());
	// 		$imagename=Yii::app()->params['filePath'].$model->name;
	//                 $image->save($imagename);
	// 		list($width, $height, $type, $attr)=@getimagesize($imagename);
	//
	// 		$model->widthxheight=$width.'x'.$height;
	// 		$model->size=filesize($imagename);
	// 		$model->type=$model->name->getType($imagename);
	// 		$model->createTime=time();
	//             }
	//             if($model->save())
	//                 $this->redirect(array('files/admin'));
