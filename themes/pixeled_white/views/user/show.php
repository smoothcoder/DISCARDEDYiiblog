<h2>
	<?php echo Yii::t('lan','View User'); ?>
	<?php echo CHtml::link("#{$model->id}",array('user/list','#'=>'c'.$model->id)); ?>
</h2>
...
<div class="profile" id="c<?php echo $model->id; ?>">
	...
	<div class="info">
		...
		<?php foreach($model->posts as $post): ?>
			<?php if( (Yii::app()->user->status!=User::STATUS_ADMIN && $post->status==Post::STATUS_PUBLISHED && $post->publishTime<time() )
