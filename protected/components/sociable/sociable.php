<?php
/*
Extension author: smoothcoder@discardedteenz.com
	Greatly Inspired from WP Plugin Sociable (Plugin URI: http://push.cx/sociable)
Author: Peter Harkins Author URI: http://push.cx
...
class sociable extends CWidget
 {

	public $post;
	public $imagepath;
	public $sociabledir;
	public $stylesheet = "sociable.css";
	...
	*/
	public $sociable_known_sites = Array(
		'BarraPunto' => Array(
	...
	/**
	* Registers the clientside widget files (css & images)
	*/
	public function init() {
		$this->sociabledir=Yii::getPathOfAlias('application.components').'/sociable/';
		// publish the files
		$this->assetstylesheet=Yii::app()->assetManager->publish($this->sociabledir.'/'.$stylesheet);
	...
	protected function registerClientScript()
	{
		...
		$cs->registerCssFile($this->assetstylesheet.'/'.$this->stylesheet);
	}
 ...
 /**
   ...
   */
	...
	public function sociable_html() {
		$active_sites = $this->get_sociable_active_sites();  // TODO Customization
		$this->imagepath=$this->assetstylesheet.'/images/';
		$html =(string) "";

		// Load the post's data
		$blogname = Yii::app()->params['title'];
		$baseurl=Yii::app()->getRequest()->getBaseUrl(true); //get the full url
 		$permalink =$baseurl.'/'.'post/'.$this->post['slug'];
		$title = CHtml::encode($this->post['title']);
		$rss = CHtml::encode($baseurl.'/site/postFeed');

		//// Grab the excerpt, if there is no excerpt, create one
		$excerpt	= $this->post->sc_getexcerpt($this->post['contentshort']);

		// Start preparing the output
		$html .= "\n<div class=\"sociable\">\n";

		// If a tagline is set, display it above the links list
		$tagline = ""; // get_option("sociable_tagline");	// MFM this is left for further implementation
		if ($tagline != "") {
		...
		*/
		...
		foreach($active_sites as $sitename) {
			...
			$site =  $this->sociable_known_sites[$sitename];
			$html .= "\t<li>";

			$url = $site['url'];
			$url = str_replace('PERMALINK', $permalink, $url);
// echo $url."##".$permalink."<br/>";
// exple site : http://www.antoineleroux.fr/wordpress/sociable-wordpress-3-4-4-fr/644
			...
			$html .= ">";
			$html .= "<img src=\"$this->imagepath{$site['favicon']}\" title=\"$sitename\" alt=\"$sitename\" class=\"sociable-hovers";
			if ($site['class'])
	...
	public function get_sociable_active_sites () {
		if (isset($this->active_sites))
